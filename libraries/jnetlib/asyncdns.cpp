    ...
    if (_this->m_cache[x].last_used && !_this->m_cache[x].resolved)
    {
      if (!nowinsock) 
      {
        ...
        else if (_this->m_cache[x].mode==1)
        {
          ...
          _this->m_cache[x].hostname = ent ? ent->h_name : "";
        }
...
int JNL_AsyncDNS::resolve(CBString hostname, unsigned long *addr)
{
  ...
  {
    if (m_cache[x].mode == 0  &&  (m_cache[x].hostname == hostname))
    {
...
int JNL_AsyncDNS::reverse(unsigned long addr, CBString &hostname)
{
  // return 0 on success, 1 on wait, -1 on unresolvable
  ...
  for (int x = 0; x < m_cache_size; x ++)
  {
    if (m_cache[x].addr==addr && m_cache[x].mode==1)
    {
      ...
      if (m_cache[x].resolved)
      {
		if (m_cache[x].hostname.length() < 1) return -1;
        hostname = m_cache[x].hostname;
        return 0;
      }
      makesurethreadisrunning();
      return 1;
