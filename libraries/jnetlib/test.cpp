...
int main(int argc, char **argv) 
{
  ...
  int part[4] = {0, 0, 0, 0};
  if (sscanf(string, "%d.%d.%d.%d", &part[0], &part[1], &part[2], &part[3]) == 4) {
    char hn[512] = "";
    while (*hn == NULL) {
      switch (dns.reverse(JNL::ipstr_to_addr(string),hn))
      {
        case 0:
          printf("Reversed in %dms: %s\n%s\n", GetTickCount()-tick, string, hn);
          return 0;

        case -1:
          printf("Error Reversing: %s\n", string);
          return -1;
      }
      Sleep(1);
    }
    ...
    while (addr == 0) {
      switch (dns.resolve(string,&addr))
      {
        case 0: 
          {
            char str[256];
            JNL::addr_to_ipstr(addr,str,256);
            printf("Resolved in %dms: %s\n%s\n", GetTickCount()-tick, string, str); 
            return 0;
          }
          break;

        case -1:
          printf("Error Resolving: %s", string);
          return -1;
      }
      Sleep(1);
    }
