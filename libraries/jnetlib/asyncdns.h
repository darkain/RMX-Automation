...
public:
  ...
  int resolve(CBString hostname, unsigned long *addr); // return 0 on success, 1 on wait, -1 on unresolvable
  int reverse(unsigned long addr, CBString &hostname); // return 0 on success, 1 on wait, -1 on unresolvable. hostname must be at least 256 bytes.

private:
  typedef struct 
  {
    __int64 last_used; // timestamp.
    char resolved;
    char mode; // 1=reverse
    CBString hostname;
    unsigned long addr;
