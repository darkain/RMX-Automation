...
static const lua_WChar *wmemchr(const lua_WChar *S, lua_WChar C, size_t N)
        {for (; 0 < N; ++S, --N)
                if (*S == C)
                        return (S);
        return (0); }
static int wmemcmp(const lua_WChar *S1, const lua_WChar *S2, size_t N)
        {for (; 0 < N; ++S1, ++S2, --N)
                if (*S1 != *S2)
                        return (*S1 < *S2 ? -1 : +1);
        return (0); }
  ...
  while (l--) {
    switch (*s) {
      case '"':  case '\\':  case '\n':
        ...
        luaL_putwchar(b, *s);
        break;
      case '\0': luaL_addlwstring(b, (const lua_WChar*)L"\\000", 4); break;
		  ...
		  case '"':  case '\\':
			...
			default:
				...
				else
				{
					char str[10];
					lua_WChar wstr[10];
					sprintf(str, "\\x%02x", (unsigned long)*s);
					translate_single_to_wide(str, wstr);
					luaL_addwstring(b, wstr);
				}
		}
...
static const lua_WChar *scanformat (lua_State *L, const lua_WChar *strfrmt,
  ...
  while (lua_WChar_chr((const lua_WChar*)L"-+ #0", *p)) p++;  /* skip flags */
    ...
    if (*strfrmt != '%')
      ...
      switch (*strfrmt++) {
        ...
        case 'o':  case 'u':  case 'x':  case 'X':
          ...
          translate_wide_to_single(form, _form);
          sprintf(_buff, _form, (unsigned int)luaL_checknumber(L, arg));
          translate_single_to_wide(_buff, buff);
          break;
