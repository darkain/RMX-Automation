...
#define lex_isspace(c)			( (0x09 <= (c) && (c) <= 0x0d) || 0x20 == (c) )
#define lex_iscntrl(c)          ( (0x00 <= (c) && (c) <= 0x1F) || (c) == 0x7F)
/*#else
#define lex_isalpha(c) iswalpha((wint_t)c)
#define lex_isdigit(c) iswdigit((wint_t)c)
#define lex_isalnum(c) iswalnum((wint_t)c)
#define lex_isspace(c) iswspace((wint_t)c)
#endif */
    ...
    switch (LS->current) {
      ...
      default: {
        if (lex_isspace(LS->current)) {
          ...
          int c = LS->current;
		  if (c >= 255)
            luaX_error(LS, "invalid wide char",
