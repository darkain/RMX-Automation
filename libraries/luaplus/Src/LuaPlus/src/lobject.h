...
#define lua_releaseupval(L,o) { UpVal* i_o2 = (o); if((--i_o2->ref)<=0) lua_releaseobject(L,(GCObject*)i_o2); }
...
#define newarray(from,to) { TObject* i_from = (from); TObject* i_to = (to); while (i_from < i_to) { newvalue(i_from++); } }
...
#define cleanarrayreverse(to,from) { TObject* i_from = (from); TObject* i_to = (to); while (i_from >= i_to) { cleanvalue(i_from--); } }
...
#define lua_addref(o)
#define lua_addreftobject(o)
#define lua_addreftable(o)
#define lua_addrefproto(o)
#define lua_addrefupval(o)
#define lua_addrefstring(o)

#define lua_release(L,o)

#define lua_releasetable(L,o)
#define lua_releaseproto(L,o)
#define lua_releaseupval(L,o)
#define lua_releasestring(L,o)

#define lua_makeobjectbackup(v)

#define newvalue(o)
#define newarray(from,to)
#define cleanvalue(o)
#define cleanarray(from,to)
#define cleanarrayreverse(to,from)
...
#define setwsvalue(obj,x) \
  { TObject *i_o=(obj); lua_makeobjectbackup(i_o); i_o->tt=LUA_TWSTRING; \
    i_o->value.gc=cast(GCObject *, (x)); lua_addreftobject(i_o); lua_release(L,&bak); \
