...
int luaO_wstr2d (const lua_WChar *s, lua_Number *result) {
  ...
  if (endptr == s) return 0;  /* no conversion */
  while (*endptr < 256 && isspace((unsigned char)*endptr)) endptr++;
  if (*endptr != '\0') return 0;  /* invalid trailing characters? */
  *result = res;
  return 1;
}
...
void lua_releaseobject(lua_State *L, GCObject* o)
{
	switch (o->gch.tt)
	{
		...
		case LUA_TWSTRING: {
			const lua_WChar* str = getwstr(&o->ts); (void)str;
			UnlinkString(L, gcototws(o));
		...
		case LUA_TUPVAL: {
			UpVal *u = gcotouv(o);
			if (u->prev)
				u->prev->gch.next = u->next;
			else {
				L->openupval = u->next;
			}
			if (u->next)
				u->next->gch.prev = u->prev;
