...
static int luaB_tonumber (lua_State *L) {
  ...
  if (base == 10) {  /* standard conversion */
    ...
    const lua_WChar *ws = luaL_checkwstring(L, 1);
	char buf[100];
    const char* s1 = buf;
	char *s2;
    unsigned long n;
    luaL_argcheck(L, 2 <= base && base <= 36, 2, "base out of range");
	if (lua_WChar_len(ws) > sizeof(buf) - 1)
	{
      lua_pushnil(L);
      return 1;
	}
	translate_wide_to_single(ws, buf);
...
static int luaB_towstring (lua_State *L) {
  ...
  switch (lua_type(L, 1)) {
    ...
    case LUA_TSTRING: {
	 ...
	 if (l == 0)
	 {
	     lua_pushwstring(L, (const lua_WChar*)L"");
	 }
	 else
	 {
		 luaL_wbuffinit(L, &b);
...
LUALIB_API int luaopen_base (lua_State *L) {
  ...
  lua_getregistry(L);
  for (i = LUA_TBOOLEAN; i < LUA_NTYPES - 1; i++)
  {
